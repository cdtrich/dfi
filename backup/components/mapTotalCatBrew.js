...
export function mapTotalCatBrew(world, data, { width, height }) {
  ...
  // get colorsScales()
  //   const fillScale = d3.scale(d3.interpolateRgb("#fffccc", colorMap[category]));
  // Define the number of steps
  const numColors = 5;

  // Create a scale using RdYlBu color interpolation
  const colorScale = d3
    .scaleSequential(d3.interpolateSpectral)
    .domain([0, numColors - 1]);

  // Generate the five colors
  const colors = d3.range(numColors).map((i) => colorScale(i));
  console.log("colors", colors);
  // Render the map
  const map = Plot.plot({
    projection: "equal-earth",
    width: width,
    title: "Common colorBrewer diverging spectral color scale",
    subtitle: "i.e. not uniquely GIFI",
    // opacity: {
    //   legend: true,
    //   range: [0, 1],
    //   domain: [min, max],
