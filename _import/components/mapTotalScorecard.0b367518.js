...
export function mapTotalScorecard(world, data, { width, height }) {
  ...
  container.style.flexDirection = "column"; // ✅ Stack map + scorecard vertically
  ...
  scorecardRow.style.flexDirection = "row"; // ✅ Ensure scorecard elements are inline
  scorecardRow.style.marginBottom = "0px";
  scorecardRow.style.justifyContent = "flex-end"; // ✅ Right-aligns the scorecard
  scorecardRow.style.height = "20px";

  // Scorecard component
  const scorecardDiv = document.createElement("div");
  scorecardDiv.style.display = "flex"; // ✅ Ensure items appear in a single line
  scorecardDiv.style.flexDirection = "row";
  scorecardDiv.style.gap = "20px"; // ✅ Spacing between values
  scorecardDiv.textContent = "Hover over a region"; // Default text
  scorecardDiv.style.justifyContent = "flex-end"; // ✅ Right-aligns the scorecard

  // Append elements to the container correctly
  container.appendChild(scorecardRow); // ✅ Scorecard follows the map
  container.appendChild(mapDiv);
  scorecardRow.appendChild(scorecardDiv); // ✅ Place scorecard inside the row
